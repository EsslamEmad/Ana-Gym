...
class UnsbscribedProgramViewController: UIViewController {
    ...
    @IBAction func didPressSubscribe(_ sender: Any) {
        guard Auth.auth.user != nil, Auth.auth.isSignedIn == true else {
            let alert = UIAlertController(title: NSLocalizedString("تسجيل الدخول", comment: ""), message: NSLocalizedString("قم بتسجيل الدخول أولا لتتمكن من الإشتراك بالبرنامج", comment: ""), preferredStyle: .alert)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "Checkout"{
            ...
            destination.url = program.mainVideo
        }
    }
